...
public struct AnyNIOSSLCustomPrivateKey: NIOSSLCustomPrivateKey, Hashable {
    ...
    @inlinable public var signatureAlgorithms: [SignatureAlgorithm] {
        return self._value.signatureAlgorithms
    }

    // This method does not need to be @inlinable for performance, but it needs to be _at least_
    // @usableFromInline as it's a protocol requirement on a @usableFromInline type.
    @inlinable public func sign(channel: Channel, algorithm: SignatureAlgorithm, data: ByteBuffer) -> EventLoopFuture<ByteBuffer> {
        return self._value.sign(channel: channel, algorithm: algorithm, data: data)
    }

    // This method does not need to be @inlinable for performance, but it needs to be _at least_
    // @usableFromInline as it's a protocol requirement on a @usableFromInline type.
    @inlinable public func decrypt(channel: Channel, data: ByteBuffer) -> EventLoopFuture<ByteBuffer> {
    ...
    // This method does not need to be @inlinable for performance, but it needs to be _at least_
    // @usableFromInline as it's a protocol requirement on a @usableFromInline type.
    @inlinable public static func ==(lhs: AnyNIOSSLCustomPrivateKey, rhs: AnyNIOSSLCustomPrivateKey) -> Bool {
